...
void CarPhysicsConfig::loadCarPhysicsFromYAML(CarPhysics& physics, const void* node_ptr) {
    ...
    if (node["turning"]) {
        ...
        if (turning["torque"]) physics.torque = turning["torque"].as<float>();
        if (turning["angular_friction"]) physics.angular_friction = turning["angular_friction"].as<float>();
        if (turning["angular_damping"]) physics.angular_damping = turning["angular_damping"].as<float>();
    }

    // Load movement properties
    if (node["movement"]) {
        const YAML::Node& movement = node["movement"];
        if (movement["max_speed"]) physics.max_speed = movement["max_speed"].as<float>();
        if (movement["max_acceleration"]) physics.max_acceleration = movement["max_acceleration"].as<float>();
    ...
    if (node["body"]) {
        ...
        if (body["density"]) physics.density = body["density"].as<float>();
        if (body["friction"]) physics.friction = body["friction"].as<float>();
    ...
    if (node["health"]) {
        ...
        if (health["collision_damage_multiplier"])
            physics.collision_damage_multiplier = health["collision_damage_multiplier"].as<float>();
    }
}
