...
void GameLoop::add_player(int id, std::shared_ptr<Queue<ServerMessage>> player_outbox)
{
    ...
    if (int(players.size()) == 0)
    {
        players[id] = PlayerData{create_player_body(INITIAL_X_POS, INITIAL_Y_POS),
    ...
    else if (int(players.size()) < MAX_PLAYERS)
    {
        ...
        players[id] = PlayerData{create_player_body(dir_x, dir_y),
...
b2Body *GameLoop::create_player_body(float x, float y)
{
    b2BodyDef bd;
    bd.type = b2_staticBody;
    bd.position.Set(INITIAL_X_POS, INITIAL_Y_POS);
