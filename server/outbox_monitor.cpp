...
void OutboxMonitor::add(int client_id, std::shared_ptr<Queue<ServerResponse>> q)
{
...
std::shared_ptr<Queue<ServerResponse>> OutboxMonitor::get_cliente_queue(int id)
{
    std::lock_guard<std::mutex> lock(mtx);
    return outboxes[id];
}
...
void OutboxMonitor::broadcast(const ServerMessage &msg)
{
    ...
    {
        q->push(ServerResponse{msg}); // Wrap en ServerResponse
    }
}
